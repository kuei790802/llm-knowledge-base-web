...
# ── Step 5: API key ──────────────────────────────────────────────────
echo ""
echo -e "${YELLOW}[5/6] Anthropic API key${NC}"
echo "  Get yours at: https://console.anthropic.com/settings/keys"
echo ""
read -s -p "  API key (input hidden): " API_KEY
echo ""

if [[ ! "$API_KEY" =~ ^sk-ant- ]]; then
  echo -e "${YELLOW}⚠ Key doesn't start with 'sk-ant-'. Double-check it's correct.${NC}"
fi
echo -e "${GREEN}✓ API key saved${NC}"

# ── Step 6: Access token (optional) ─────────────────────────────────
echo ""
echo -e "${YELLOW}[6/6] Cloudflare tunnel access token (optional)${NC}"
...
# Generate a random token if user wants tunnel but skipped this
