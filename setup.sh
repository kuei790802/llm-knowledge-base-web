...
if command -v claude &> /dev/null; then
  ...
  if claude auth status &> /dev/null; then
    ...
    echo -e "${YELLOW}    ⚠ Not logged in — run: claude auth login${NC}"
  fi
  AVAILABLE_CLIS+=("claude")
else
  echo -e "${YELLOW}  ○ Claude Code not found (npm i -g @anthropic-ai/claude-code)${NC}"
fi
...
if command -v codex &> /dev/null; then
  ...
  echo -e "${YELLOW}  ○ Codex not found (npm i -g @openai/codex)${NC}"
fi
...
if command -v gemini &> /dev/null; then
  ...
  echo -e "${YELLOW}  ○ Gemini CLI not found (npm i -g @google/gemini-cli)${NC}"
fi

if [ ${#AVAILABLE_CLIS[@]} -eq 0 ]; then
  echo -e "${RED}✗ No CLI providers found. Install at least one:${NC}"
...
# ── Step 3: Vault path ───────────────────────────────────────────────
echo ""
echo -e "${YELLOW}[3/7] Obsidian Vault location${NC}"
...
echo ""
echo -e "${YELLOW}[4/7] Knowledge domains${NC}"
...
echo ""
echo -e "${YELLOW}[5/7] Cloudflare tunnel access token (optional)${NC}"
...
# ── Step 6: Default CLI provider ────────────────────────────────────
echo ""
echo -e "${YELLOW}[6/7] Default CLI provider${NC}"

CLI_PROVIDER="${AVAILABLE_CLIS[0]}"
if [ ${#AVAILABLE_CLIS[@]} -gt 1 ]; then
  echo "  Available providers:"
  for i in "${!AVAILABLE_CLIS[@]}"; do
    echo "    $((i+1)). ${AVAILABLE_CLIS[$i]}"
  done
  echo ""
  read -p "  Default CLI [${AVAILABLE_CLIS[0]}]: " CLI_CHOICE
  if [ -n "$CLI_CHOICE" ]; then
    # Accept number or name
    if [[ "$CLI_CHOICE" =~ ^[0-9]+$ ]] && [ "$CLI_CHOICE" -ge 1 ] && [ "$CLI_CHOICE" -le ${#AVAILABLE_CLIS[@]} ]; then
      CLI_PROVIDER="${AVAILABLE_CLIS[$((CLI_CHOICE-1))]}"
    else
      CLI_PROVIDER="$CLI_CHOICE"
    fi
  fi
fi
echo -e "${GREEN}✓ Default CLI: $CLI_PROVIDER${NC}"

# Ask for API keys if needed
OPENAI_API_KEY=""
GEMINI_API_KEY=""

if [[ " ${AVAILABLE_CLIS[*]} " =~ " codex " ]] && [ -z "$OPENAI_API_KEY" ]; then
  echo ""
  read -s -p "  OpenAI API key (optional, press Enter to skip): " OPENAI_API_KEY_INPUT
  echo ""
  OPENAI_API_KEY="${OPENAI_API_KEY_INPUT}"
fi

if [[ " ${AVAILABLE_CLIS[*]} " =~ " gemini " ]] && [ -z "$GEMINI_API_KEY" ]; then
  echo ""
  read -s -p "  Gemini API key (optional, press Enter to skip): " GEMINI_API_KEY_INPUT
  echo ""
...
# ── Step 7: Write .env ──────────────────────────────────────────────
echo ""
echo -e "${YELLOW}[7/7] Writing configuration...${NC}"
